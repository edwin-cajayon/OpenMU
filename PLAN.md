...
|---|------|----------|--------|
| G1 | Get the OpenMU server running locally on Windows | P0 | ✅ Done (2026-04-26, real PostgreSQL DB) |
| G2 | Connect an MU Online Season 6 EP3 ENG client and log in with a test account | P0 | ✅ Done (2026-04-26, end-to-end with persistence) |
...
| `scripts/windows/dev-reset-db.ps1` | Confirm-and-reinit helper (wraps `dev-run.ps1 -Reinit`). **Always run `db-backup.ps1` first.** |
| `scripts/windows/db-backup.ps1` | `pg_dump -F c` of the `openmu` database to `local/db-backups/openmu-YYYYMMDD-HHmmss.dump`. Retention defaults to keep newest 14. Verifies the dump with `pg_restore --list`. Auto-detects PG 17/16/15 install. |
| `scripts/windows/db-restore.ps1` | Restore the newest (or named) dump back into `openmu` with `pg_restore --clean --if-exists`. Confirmation prompt. `-NoOwner` skips role grants. **Stop the server first.** |
...
| `local/db-backups/` | `pg_dump` snapshots produced by `scripts/windows/db-backup.ps1`. Contain a full copy of every account / character / config row, so they must never leave the machine via Git. |
...
- **2026-04-27 — AI (Claude) — Step 21 closed: `6GOD` promoted to GameMaster. Took pre-change snapshot via `db-backup.ps1` (3rd dump, `openmu-20260427-183204.dump`). Server was already stopped, so no reconnect dance. `UPDATE data."Character" SET "CharacterStatus" = 32 WHERE "Name" = '6GOD'` returned `UPDATE 1`. Audit query confirms total GM characters in DB is now 8: `6GOD/edwin` (just promoted) + the 5 `testgm*` chars under `testgm` (DK/DL/DW/Elf/MG) + the 2 `testgm2*` chars under `testgm2` (Rf/Sum) — exactly matching `GMchat.md` §1. Updated faq.md: custom-account row reflects new GM status, Step 21 added (done), Step 22 (Phase D VPS) renumbered. All slash-commands from `GMchat.md` are now live for `6GOD` on next login. Open-question about `Account.State=GameMaster` not propagating to created characters is now empirically resolved as a workaround (manual UPDATE after-the-fact); the `Why` (intentionality of the upstream behaviour) is still TBD pending a read of `src/GameLogic/PlayerActions/Character/CreateCharacterAction.cs`.**
- **2026-04-27 — AI (Claude) — Step 20 closed: pg_dump backup workflow set up. Verified the post-Phase-B DB state intact (21 accounts including `edwin`, 77 characters, `6GOD` row matches the FAQ snapshot exactly: Experience=5671, LevelUpPoints=30, CharacterStatus=0). Took first snapshot to `local/db-backups/openmu-20260427-181727.dump` (1.13 MB, 812 TOC entries, 120 TABLE DATA entries — all critical tables present including `data.Account`, `data.Character`, `config.GameConfiguration`). Wrote `scripts/windows/db-backup.ps1` (custom-format dump, retention=14, verifies with `pg_restore --list`, auto-detects PG 17/16/15 install) and `scripts/windows/db-restore.ps1` (picks newest dump by default, `--clean --if-exists`, confirmation prompt, `-NoOwner` flag for cases where role logins were dropped). Updated `scripts/windows/README.md` with usage + Task Scheduler one-liner; updated `faq.md` Step 20 (done), Working Commands section (helper scripts now preferred over the manual `pg_dump` snippet; fixed restore example to use `--clean --if-exists` over the bare `-c` flag), and Important Paths (added the two scripts + `local/db-backups/`). Phase C unblocked — any tuning experiment that needs `-reinit` is now safe.**
- **2026-04-26 — AI (Claude) — Phase B fully verified end-to-end. Step 17c (admin panel browser walkthrough): user created custom account `edwin` via Accounts → Create-new modal in admin panel; row landed in `data."Account"` with BCrypt-hashed password and `State=GameMaster`. Step 18 (persistence test): user logged in via launcher (windowed mode toggled via `HKCU:\SOFTWARE\WebZen\Mu\Config\WindowMode = 1`), created Dark Knight character `6GOD` (4 chars — Webzen client hardcodes 4-char minimum even though server regex allows 3), played to EXP=5671 + LevelUpPoints=30, confirmed character + progression persist across disconnect/reconnect and server restart. Goals G1 + G2 marked Done. Three new gotchas documented in faq.md (3-letter-name client lock, no in-game account creation, registry-based window mode). Side-quest `GMchat.md` from prior session is the natural Phase C launchpad. Server still running on PID 20552 since 10:41 AM (no restart needed during Phase B work).**
- **2026-04-26 — AI (Claude) — Phase B resume after break. Verified Postgres 17 service `Running`, `openmu` DB intact (20 seed accounts, `GameConfiguration` populated), no OpenMU process or ports occupied. Started server with `dotnet run --project Startup -c Release --no-build -- -autostart -deamon` and `RESOLVE_IP=loopback`. Server up in 12.6 s; all 12 listeners green (80, 44405-44408, 55901-55906, 55980); admin panel returns HTTP 200 OK. Discovered there are now 4 Connect Servers (44405-44408), not the canonical 2 — configured in admin panel during the previous session. Updated faq.md: split former Step 17 into 17a (server up, ports green ✓), 17b (DB sanity check ✓, noted EF EnsureCreated fallback rather than Migrate path), 17c (manual browser walkthrough — pending). Goals G1 + G2 effectively achieved, just need user to browser-verify admin panel UI and run launcher login (Step 18) to formally close them out. GMchat.md from prior session is correct prep for Phase C plug-in toggling.**
